...
import { HistoryOptions, parseAsString, useQueryStates } from "nuqs";

import { TransitionStartFunction, useMemo } from "react";

import { RessourceFilter, allOperators, buildPrismaWhere } from "./ressource";

export type Operator = RessourceFilter.Operator;

export const operators: Operator[] = allOperators;
type Options = {
    /**
...
export type UseRessourceFilterProps = {
    fieldMap: RessourceFilter.FieldMap;
} & Options;

export function useRessourceFilter({
    fieldMap,
    ...options
    ...
    const where = useMemo(() => {
        return buildPrismaWhere(filters, fieldMap);
    }, [filters, fieldMap]);
